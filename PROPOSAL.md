...
| PUT         |   | '/auth/users/signup' |   | Read           |   | yes               |   | the info when user fills in                  |   | redirect to anime list                                                              |
| GET         |   | '/auth/users/login   |   | Read           |   | no                |   | the user log in page                         |   | input button redirects to PUT auth/users/login                                      |
| PUT         |   | 'auth/users/login'   |   | Read           |   | yes               |   | the info when user fills in log in form      |   | redirect to anime list                                                              |
| GET         |   | 'auth/animes'        |   | Read           |   | no                |   | shows your full list of anime                |   | each anime is clickable to redirect to their specific page for editing and deleting |
| GET         |   | 'auth/animes:id'     |   |                |   |                   |   |                                              |   |                                                                                     |
|             |   |                      |   |                |   |                   |   |                                              |   |                                                                                     |
